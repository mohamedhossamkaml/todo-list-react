...
function Navbar({ toggleSidebar }) {
  ...
  const staticNav = [
    { id: 'home', label: 'home', color: '#000' }
  ];

  const dynamicNav = categories.map(({ id, label, color }) => ({
    id,
    label,
    color
  }));
    ...
    <motion.header data-aos="fade-down"
      ...
      <nav className="container mx-auto flex items-center  dark:text-deepMint_100 justify-between px-6 py-4">
          ...
          {navItems.map((item, i) => (
            ...
            >
              <MotionLink
                ...
                className={`px-2 py-1 rounded-md text-sm font-medium transition ${activeLink === item.label
        ...
        <div className="flex items-center gap-4">
          ...
          <Link
            to="/categories"
            className="bg-blue-600 text-white px-2 py-2 rounded hover:bg-blue-700 transition duration-300"
          >
            📁 إدارة التصنيفات
          </Link>
          ...
          <button
            ...
            className="bg-green-500 text-white px-1 py-2 rounded hover:bg-green-600"
          >
            ➕ Add Categories
          </button>
