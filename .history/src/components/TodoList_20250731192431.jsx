...
function TodoList({ category }) {
  ...
  return (
    <motion.div
      ...
      initial="hidden"
      animate="visible"
      className="space-y-8 text-deepMint_900 dark:text-white px-4"
    >
      <form
        onSubmit={handleSubmit}
        className="flex flex-col text-deepMint_900 dark:text-white md:flex-row items-center gap-4 justify-center mb-4"
      >
        <input
          value={input}
          onChange={(e) => setInput(e.target.value)}
          className="flex-1 bg-deepMint_500 dark:bg-deepMint_600 p-3 border rounded-lg shadow-sm "
